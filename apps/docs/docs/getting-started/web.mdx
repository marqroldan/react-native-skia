...
PDF output on Web requires the PDF-enabled CanvasKit JavaScript/WASM pair
generated by this fork. A package produced by the `prepack` hook includes the
pair under `dist/canvaskit-web-pdf/`; run `npx setup-skia-web` so the matching
WASM file is copied to the application's public folder. The default
`LoadSkiaWeb()` loader then exposes:
...
LoadSkiaWeb().then(async () => {
  ...
  if (!Skia.PDF.isAvailable()) {
    throw new Error("This Web build does not include PDF-enabled CanvasKit");
  }
});
```

When hosting the generated files on a CDN, point `locateFile` at the same
PDF-enabled artifact directory. Do not point the default loader at the stock
`canvaskit-wasm` binary, because it does not contain the PDF bindings:

```tsx
LoadSkiaWeb({
  locateFile: (file) => `https://cdn.example.com/skia-pdf/${file}`,
});
```

If an application intentionally uses stock CanvasKit, pass its initializer
explicitly; PDF remains unavailable for that build:

```tsx
import CanvasKitInit from "canvaskit-wasm/bin/full/canvaskit";

LoadSkiaWeb({ CanvasKitInit });
```
