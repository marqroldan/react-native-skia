...
class JsiSkPDFFactory : public JsiSkNativeObject<JsiSkPDFFactory> {
public:
  static constexpr const char *CLASS_NAME = "PDFFactory";
  ...
  JSI_HOST_FUNCTION(MakeDocument) {
    ...
    return makeJsiObject(runtime, std::make_shared<JsiSkPDFDocument>(
                                      getContext(), std::move(document)));
#else
    throw jsi::JSError(
        runtime,
  ...
  static void definePrototype(jsi::Runtime &runtime, jsi::Object &prototype) {
    installHostMethod(runtime, prototype, "MakeDocument",
                      &JsiSkPDFFactory::MakeDocument);
    installHostMethod(runtime, prototype, "isAvailable",
                      &JsiSkPDFFactory::isAvailable);
  }

  size_t getMemoryPressure() override { return 1024; }

  explicit JsiSkPDFFactory(std::shared_ptr<RNSkPlatformContext> context)
      : JsiSkNativeObject<JsiSkPDFFactory>(std::move(context)) {}
