  ...
  itRunsE2eOnly("enforces the document lifecycle", async () => {
    const results = await surface.eval((Skia) => {
      ...
      return {
        ...
        // A canvas whose page has ended is detached from the page and
        // redirected at a throwaway sink: drawing into it is discarded
        // rather than writing through a dangling pointer, and the document
        // itself stays usable.
        staleCanvas: capture(() => {
          const doc = Skia.PDF.MakeDocument();
    ...
    expect(results.staleCanvas).toBe("no error thrown");
